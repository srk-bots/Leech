...
async def send_photo(
    message, photo, caption="", buttons=None, parse_mode=enums.ParseMode.HTML
):
    """Send a photo message

    Args:
        message: Message to reply to or chat ID to send to
        photo: Photo to send (file path, URL, or bytes)
    ...
    try:
        ...
        LOGGER.warning(str(f))
        await sleep(f.value * 1.2)
        return await send_photo(message, photo, caption, buttons, parse_mode)
    except Exception as e:
...
async def send_video(
    message, video, caption="", buttons=None, parse_mode=enums.ParseMode.HTML
):
    """Send a video message

    Args:
        message: Message to reply to or chat ID to send to
        video: Video to send (file path, URL, or bytes)
    ...
    try:
        ...
        LOGGER.warning(str(f))
        await sleep(f.value * 1.2)
        return await send_video(message, video, caption, buttons, parse_mode)
    except Exception as e:
...
async def send_audio(
    message, audio, caption="", buttons=None, parse_mode=enums.ParseMode.HTML
):
    """Send an audio message

    Args:
        message: Message to reply to or chat ID to send to
        audio: Audio to send (file path, URL, or bytes)
    ...
    try:
        ...
        LOGGER.warning(str(f))
        await sleep(f.value * 1.2)
        return await send_audio(message, audio, caption, buttons, parse_mode)
    except Exception as e:
...
async def send_document(
    message, document, caption="", buttons=None, parse_mode=enums.ParseMode.HTML
):
    """Send a document message

    Args:
        message: Message to reply to or chat ID to send to
        document: Document to send (file path, URL, or bytes)
    ...
    try:
        ...
        LOGGER.warning(str(f))
        await sleep(f.value * 1.2)
        return await send_document(message, document, caption, buttons, parse_mode)
    except Exception as e:
