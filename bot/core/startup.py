...
async def validate_chat_ids():
    """Validate that the bot has access to all configured chat IDs."""
    from bot import LOGGER
    from bot.core.config_manager import Config

    # Validate LEECH_DUMP_CHAT
    if Config.LEECH_DUMP_CHAT:
        LOGGER.info(
            f"Validating {len(Config.LEECH_DUMP_CHAT)} leech dump chat IDs..."
        )
        valid_dump_chats = []
        for chat_id in Config.LEECH_DUMP_CHAT:
            try:
                # Skip validation for usernames as they're resolved at runtime
                if isinstance(chat_id, str) and chat_id.startswith("@"):
                    valid_dump_chats.append(chat_id)
                    LOGGER.info(f"Added username to leech dump chats: {chat_id}")
                    continue
                ...
                LOGGER.info(
                    f"Successfully validated leech dump chat: {chat_title} ({chat_id})"
                )
            except Exception as e:
                LOGGER.error(f"Failed to validate leech dump chat ID {chat_id}: {e}")
                LOGGER.warning(
                    f"Removing invalid chat ID {chat_id} from LEECH_DUMP_CHAT"
                )

        # Update Config with only valid chat IDs
        if len(valid_dump_chats) != len(Config.LEECH_DUMP_CHAT):
            LOGGER.warning(
                f"Updated LEECH_DUMP_CHAT from {len(Config.LEECH_DUMP_CHAT)} to {len(valid_dump_chats)} valid chats"
            )
            Config.LEECH_DUMP_CHAT = valid_dump_chats

    # Validate LOG_CHAT_ID
    if Config.LOG_CHAT_ID:
        LOGGER.info(f"Validating {len(Config.LOG_CHAT_ID)} log chat IDs...")
        valid_log_chats = []
        for chat_id in Config.LOG_CHAT_ID:
            try:
                # Skip validation for usernames as they're resolved at runtime
                if isinstance(chat_id, str) and chat_id.startswith("@"):
                    valid_log_chats.append(chat_id)
                    LOGGER.info(f"Added username to log chats: {chat_id}")
                    continue
                ...
                chat_title = getattr(chat, "title", str(chat_id))
                LOGGER.info(
                    f"Successfully validated log chat: {chat_title} ({chat_id})"
                )
            except Exception as e:
                LOGGER.error(f"Failed to validate log chat ID {chat_id}: {e}")
                LOGGER.warning(
                    f"Removing invalid chat ID {chat_id} from LOG_CHAT_ID"
                )

        # Update Config with only valid chat IDs
        if len(valid_log_chats) != len(Config.LOG_CHAT_ID):
            LOGGER.warning(
                f"Updated LOG_CHAT_ID from {len(Config.LOG_CHAT_ID)} to {len(valid_log_chats)} valid chats"
            )
            Config.LOG_CHAT_ID = valid_log_chats


async def start_bot():
    # Validate chat IDs
    if hasattr(TgClient, "bot") and TgClient.bot is not None:
        await validate_chat_ids()
