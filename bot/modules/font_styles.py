...
def get_usage_examples_page():
    ...
    msg += "8. <b>Unlimited nesting with multiple styles:</b>\n"
    msg += "   • <code>{{{{{filename}bold}italic}code}underline}strike</code> - Five nested styles\n"
    msg += "   • <code>{{{{{{filename}Roboto:700}bold}italic}code}underline}strike</code> - Six nested styles\n"
    msg += "   • <code>{{{{{{{filename}🔥}bold}italic}code}underline}strike}Roboto</code> - Seven nested styles\n\n"
    msg += "9. <b>Complete caption examples:</b>\n"
...
def get_google_fonts_page():
    ...
    msg += "• Unlimited nesting of styles is supported - you can combine any number of styles in any order\n"
