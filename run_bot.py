...
def find_module_dir():
    ...
    for module in potential_modules:
        if os.path.isdir(module) and os.path.exists(os.path.join(module, "__main__.py")):
...
def import_and_run_module(module_dir):
    ...
    try:
        ...
        try:
            module = __import__(f"{module_dir}.__main__", fromlist=['*'])
...
def run_bot():
    ...
    for root, dirs, _ in os.walk(module_dir):
        for d in dirs:
            ...
            if not os.path.exists(init_file):
                logger.info(f"Creating missing __init__.py in {os.path.join(root, d)}")
    ...
    try:
        ...
        bot_loop = getattr(bot_loop_module, 'bot_loop')
        ...
        exec(f"from {module_dir}.helper.ext_utils.bot_utils import create_help_buttons; create_help_buttons()")
        exec(f"from {module_dir}.helper.listeners.aria2_listener import add_aria2_callbacks; add_aria2_callbacks()")
